...
test('release metadata is v0.5.3', () => {
  assert.equal(JSON.parse(read('manifest.json')).version, '0.5.3');
  assert.equal(JSON.parse(read('package.json')).version, '0.5.3');
  assert.match(read('index.js'), /VERSION = '0\.5\.3'/);
});
