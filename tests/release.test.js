...
test('all release metadata and runtime VERSION say 0.3.0', () => {
  assert.equal(JSON.parse(read('manifest.json')).version, '0.3.0');
  assert.equal(JSON.parse(read('package.json')).version, '0.3.0');
  assert.match(read('src/constants.js'), /VERSION = '0\.3\.0'/);
  assert.match(read('style.css'), /^\/\* Inventory Block v0\.3\.0 \*\//);
  assert.match(read('README.md'), /Inventory Block v0\.3\.0/);
});

test('changelog retains prior hardening and documents v0.3.0 history inspection', () => {
...
test('v0.3.0 keeps resource accounting and adds bounded history tooling', () => {
