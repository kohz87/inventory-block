...
test('all release metadata and runtime VERSION say 0.2.7', () => {
  assert.equal(JSON.parse(read('manifest.json')).version, '0.2.7');
  assert.equal(JSON.parse(read('package.json')).version, '0.2.7');
  assert.match(read('src/constants.js'), /VERSION = '0\.2\.7'/);
  assert.match(read('style.css'), /^\/\* Inventory Block v0\.2\.7 \*\//);
  assert.match(read('README.md'), /Inventory Block v0\.2\.7/);
  assert.match(read('TEST-REPORT.md'), /v0\.2\.7 Hotfix/);
});

test('changelog retains pipeline hardening and documents 0.2.7 aliases', () => {
  const changelog=read('CHANGELOG.md');
  assert.match(changelog,/## 0\.2\.7/);
...
test('0.2.7 protocol keeps Megumin-safe placement and explicit canonical op grammar', () => {
