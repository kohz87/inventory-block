...
test('release metadata, runtime version, and interceptor are v0.3.7',()=>{
  assert.equal(manifest.version,'0.3.7');
  assert.equal(pkg.version,'0.3.7');
  assert.match(constants,/VERSION = '0\.3\.7'/);
...
test('v0.3.5 has no fake prompt slot or global live extension prompt',()=>{
...
test('completed foreground messages reconcile only after completion signals',()=>{
  ...
  assert.match(index,/generateRaw/);
