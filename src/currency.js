...
If a currency balance is stored directly in Quantity, use adjust_item as normal. When the spendable balance is stored in an item's Remark rather than Quantity, keep the container Quantity unchanged and use edit_item to replace the Remark with the newly calculated balance. Example: Coin Pouch quantity "1" with remark "100 Gold", after spending 15 Gold, must emit an edit_item for that exact Coin Pouch with remark "85 Gold". Receiving 20 Gold from that state must set remark "120 Gold".
A zero balance remains a valid container item when the balance is in Remark, for example remark "0 Gold". Never produce a negative balance. If the authoritative balance cannot cover a payment, do not treat that payment as completed.
