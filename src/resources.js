...
If the changing amount is stored directly in Quantity as a plain number, use adjust_item.
Use adjust_resource ONLY when the exact authoritative Remark contains one and only one numeric amount and the completed event gives a numeric signed change in that same unit. Example: Coin Pouch quantity "1" with remark "100 Gold", after spending 15 Gold, use adjust_resource by -15 so the backend produces "85 Gold". Food quantity "1" with remark "About 7 days", after one established day of consumption, use adjust_resource by -1 so the backend preserves the approximation wording as "About 6 days". Set deleteAtZero:true only when the row is the consumable stock itself; leave it false/omitted for durable containers such as a Coin Pouch.
NEVER use adjust_resource when the exact current Remark contains zero numeric amounts, more than one numeric amount, or a numeric range. Examples that MUST NOT use adjust_resource: "Several days", "About a week", "5-7 days", "5–7 days", and "2 meals / 3 days". For those semantic/range/multi-number Remarks, use edit_item only when the response establishes the complete intended replacement Remark. If the new semantic Remark cannot be determined safely from the completed event, emit no operation for that row rather than guessing or inventing precision.
For other non-numeric or semantic Remark states, use edit_item. A Waterskin quantity "1" may move from remark "Full" to "Half full" or "Empty" as its contents are actually used.
Preserve the authoritative unit, wording, range semantics, and approximation style instead of inventing false precision. Do not convert words such as "several" or "about a week" into an invented numeric amount merely to use adjust_resource. Only apply changes established as completed in this response; planned, attempted, negotiated, interrupted, or failed actions do not consume or grant resources unless the response explicitly establishes that they did.
